  ...
  return (
    <AuthGate>
      <AppShell title={trip?.title ?? 'Itinerary View'} subtitle="Review your structured trip plan and share it publicly when ready.">
        ...
        {trip ? (
          <>
            <MotionSection className="rounded-2xl border border-white/8 bg-slate-900 p-5" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ type: 'spring', stiffness: 240, damping: 24 }}>
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <p className="text-sm text-slate-500">
                    {formatDate(trip.startDate)} - {formatDate(trip.endDate)}
                  </p>
                  <h2 className="text-2xl font-semibold text-slate-100 sm:text-[1.75rem]">{trip.title}</h2>
                  <p className="text-sm text-slate-400 sm:max-w-2xl">{trip.description || 'No description yet.'}</p>
                </div>
                <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap lg:justify-end">
                  <Link className="rounded-full border border-white/10 px-3 py-1 text-sm" href={`/trips/${trip.id}/builder`}>
                    Open builder
                  </Link>
                  <Link className="rounded-full border border-white/10 px-3 py-1 text-sm" href={`/trips/${trip.id}/budget`}>
                    Budget breakdown
                  </Link>
                  <button
              ...
              <div className="mt-4 flex flex-col gap-2 text-sm text-slate-300 sm:flex-row sm:flex-wrap sm:gap-4">
              ...
              {shareUrl ? (
                <p className="mt-4 rounded-xl bg-slate-900/50 p-3 text-sm text-slate-300">
                  Public link:{' '}
            ...
            <MotionSection className="mt-6 rounded-2xl border border-white/8 bg-slate-900 p-5" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.15 }} transition={{ type: 'spring', stiffness: 240, damping: 24 }}>
              <h3 className="text-lg font-semibold text-slate-100">Day-wise Stops</h3>
              {stopsQuery.isLoading ? <p className="mt-3 text-sm text-slate-400">Loading stops...</p> : null}
              {stops.length === 0 && !stopsQuery.isLoading ? (
                <p className="mt-3 text-sm text-slate-400">No stops yet. Add cities in itinerary builder.</p>
              ) : null}
            ...
            <AnimatedModal open={Boolean(shareUrl)} title="Share link ready" onClose={() => setShareUrl(null)}>
              <p className="text-sm text-slate-400">Anyone with this link can view the public version of your trip.</p>
              {shareUrl ? (
                <Link className="mt-3 block break-all rounded-xl bg-slate-900/50 p-3 text-sm font-medium text-teal-700" href={shareUrl} target="_blank">
                  {shareUrl}
                </Link>
              ) : null}
            </AnimatedModal>
