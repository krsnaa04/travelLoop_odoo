        ...
        <motion.div className="grid gap-5 lg:grid-cols-[1.4fr_0.6fr]" variants={stagger} initial="initial" animate="animate">
          ...
          >
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href="/trips/new" className="traveloop-button-secondary bg-white/95">
                Plan new trip
              </Link>
              <Link href="/trips" className="traveloop-pill border-white/10 bg-white/10 text-white backdrop-blur-sm">
                View trip library
              </Link>
            </div>
          </MotionSection>
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            <MotionArticle className="traveloop-card rounded-[1.35rem] p-4" variants={itemMotion} whileHover={{ y: -4, scale: 1.01 }}>
              ...
              <p className="mt-2 text-3xl font-semibold tracking-tight text-slate-900">{trips.length}</p>
            </MotionArticle>
            <MotionArticle className="traveloop-card rounded-[1.35rem] p-4" variants={itemMotion} whileHover={{ y: -4, scale: 1.01 }}>
              <p className="text-sm font-medium text-slate-500">Upcoming trip</p>
              <p className="mt-2 text-xl font-semibold text-slate-900">{nextTrip ? nextTrip.title : 'None yet'}</p>
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h3 className="mt-2 text-xl font-semibold tracking-tight text-slate-900">Your latest itineraries</h3>
            </div>
              ...
              <MotionArticle key={trip.id} className="traveloop-card rounded-[1.4rem] p-4" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ type: 'spring', stiffness: 240, damping: 24 }} whileHover={{ y: -4, scale: 1.01 }}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <h4 className="mt-1 text-lg font-semibold text-slate-900">{trip.title}</h4>
                  </div>
