    ...
    <AuthGate>
      <AppShell title="Activity Search" subtitle="Browse curated activities by category, city, and estimated cost.">
        ...
        <div className="mt-4 grid gap-3">
          {activities.map((item) => (
            <article key={item.id} className="rounded-xl border border-black/8 bg-white p-4">
              ...
              <p className="text-sm text-slate-600">{item.cityName || 'Multi-city activity'}</p>
              <p className="mt-2 text-xs text-slate-600">
                Duration {item.defaultDurationMinutes} min | Est. cost {item.estimatedCost} | Popularity {item.popularityScore}
              </p>
              <p className="mt-2 text-sm text-slate-700">{item.description || 'No description available.'}</p>
            </article>
          ))}
        </div>
      </AppShell>
